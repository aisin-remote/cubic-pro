...
<div class="container">
    <div class="row">
        <div class="col-xs-12">
            <div class="page-title-box">
                <h4 class="page-title">Update BOM Semi Finish Good</h4>
                <ol class="breadcrumb p-0 m-0">
                    <li><a href="{{ route('bom.index') }}">Upload BOM Semi Finish Good</a></li>
                    <li class="active">
                        Update BOM Semi Finish Good
                    </li>
                </ol>
    ...
    <div class="row">
        <form id="form-add-edit" action="{{ route('bom_semi.update', $bom_semi->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="col-md-12">
                <div class="card-box">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="part_id" class="select2" data-placeholder="Select Part Number" required="required" value="{{ $bom_semi->parts->part_number }}">
                                ...
                                <option value="{{ $part->id }}" {{ $part->id == $bom_semi->part_id ? 'selected=selected' : '' }}>{{ $part->part_number }} - {{ $part->part_name }}</option>
                                @endforeach
                       ...
                       <div class="form-group">
                            ...
                            <select name="supplier_id" class="select2" data-placeholder="Select Supplier Code" required="required">
                               ...
                               @foreach ($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" {{ $supplier->id == $bom_semi->supplier_id ? 'selected=selected' : '' }}>{{ $supplier->supplier_code }} - {{ $part->supplier_name }}</option>
                                @endforeach
                            </select>
                            <span class="help-block"></span>
                       </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Create Detail Bom Semi Finish Good</h4>
            </div>
