...
class CapexController extends Controller
{
	...
	public function import(Request $request)
	{
		...
		// $path = $file->move(public_path('storage/uploads'), $name);
		...
		if ($request->hasFile('file')) {
			...
			foreach ($sheet->getRowIterator() as $row) {
				...
				$dep = $sheet->getCell("A$sc")->getValue();
				$budget_no = $sheet->getCell("B$sc")->getValue();
				$sap = $sheet->getCell("C$sc")->getValue();
				$equipment = $sheet->getCell("D$sc")->getValue();
				$qty = $sheet->getCell("E$sc")->getValue();
				$price = $sheet->getCell("F$sc")->getValue();
				$gr = $sheet->getCell("G$sc")->getValue();

				if ($dep == null) {
					break;
				}
				$datas[$a]['dep'] = $dep;
				$datas[$a]['budget_no'] = $budget_no;
				$datas[$a]['sap_cc_code'] = $sap;
				$datas[$a]['equipment_name'] = $equipment;
				$datas[$a]['qty'] = $qty;
				$datas[$a]['price'] = $price;
				$datas[$a]['gr'] = $gr;
